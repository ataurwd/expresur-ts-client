...
export default function TiendaProduct() {
  ...
  const filteredAndSorted = products
    ...
    .filter((p) => selectedRating === null || Math.round(p.rating) === selectedRating)
    .filter((p) => p.price <= priceRange)
    .filter((p) => !selectedCategory || p.category === selectedCategory)
    .filter((p) => !selectedOrganization || p.organization === selectedOrganization)
      ...
      <div className="min-h-screen">
        <div className="max-w-7xl mx-auto flex gap-8 px-6 py-8">
          ...
          <aside className="w-80 space-y-8">
            ...
            {/* Price Range */}
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-green-700 mb-4">Price range</h3>
              <div className="space-y-4">
                ...
                <div className="flex justify-between text-sm text-gray-600">
                  <span>$0</span>
                  <span className="font-semibold text-orange-600">${priceRange}</span>
                </div>
              </div>
            </div>
            ...
            {/* Category */}
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-green-700 mb-4">Category</h3>
              <div className="space-y-3">
                {["Sneakers", "Boots", "T-Shirt", "Hoodie", "Jeans", "Jacket"].map((cat) => (
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-green-700 mb-4">Organization</h3>
              <div className="space-y-3">
                {["Nike", "Adidas", "Puma", "Champion", "Levi's", "Vans", "Timberland"].map(
                  (org) => (
            ...
            <div className="grid grid-cols-3 gap-6 mt-5">
              {filteredAndSorted.map((product) => (
                ...
                >
                  ...
                  <div className="p-6">
                    <h4 className="text-lg font-medium text-gray-800 mb-2">{product.title}</h4>
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-2xl font-bold text-orange-600">
                        ${product.price.toFixed(2)}
                      </span>
                      <div className="flex ml-auto">{renderStars(product.rating)}</div>
                    </div>
                  </div>
                </div>
              ))}
