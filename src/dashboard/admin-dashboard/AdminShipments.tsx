...
// Updated dummy data with email + lockerId
const initialData: Pickup[] = [
  { id: "PK-0047", customer: "María González", email: "maria.gonzalez@email.es", lockerId: "LCK-127A", phone: "+34 612 345 678", origin: "Barcelona, Spain", destination: "Havana, Cuba", date: "2025-02-03", status: "Pending" },
  { id: "PK-0046", customer: "Carlos Pérez", email: "carlos.perez@cubamail.cu", lockerId: "LCK-089B", phone: "+53 5 123 4567", origin: "Santiago de Cuba", destination: "Miami, USA", date: "2025-02-01", status: "Assigned" },
  { id: "PK-0045", customer: "Ana López", email: "ana.lopez@outlook.com", lockerId: "LCK-312C", phone: "+34 678 901 234", origin: "Valencia, Spain", destination: "Camagüey, Cuba", date: "2025-01-30", status: "In Transit" },
  { id: "PK-0044", customer: "Luis Fernández", email: "luis.fernandez@gmail.com", lockerId: "LCK-045D", phone: "+1 305 555 0198", origin: "Miami, USA", destination: "Havana, Cuba", date: "2025-01-29", status: "Completed" },
  { id: "PK-0043", customer: "Elena Martínez", email: "elena.martinez@proton.me", lockerId: "LCK-201E", phone: "+34 654 321 987", origin: "Madrid, Spain", destination: "Holguín, Cuba", date: "2025-02-05", status: "Pending" },
  { id: "PK-0042", customer: "Roberto Díaz", email: "roberto.diaz@yahoo.es", lockerId: "LCK-156F", phone: "+53 5 987 6543", origin: "Varadero, Cuba", destination: "Madrid, Spain", date: "2025-01-27", status: "Assigned" },
  { id: "PK-0041", customer: "Sofia Rivera", email: "sofia.rivera@icloud.com", lockerId: "LCK-278G", phone: "+34 699 112 233", origin: "Málaga, Spain", destination: "Cuba (Domestic)", date: "2025-02-02", status: "Pending" },
  { id: "PK-0040", customer: "Jorge Herrera", email: "jorge.herrera@usa.net", lockerId: "LCK-390H", phone: "+1 786 444 5566", origin: "New York, USA", destination: "Havana, Cuba", date: "2025-01-31", status: "In Transit" },
  { id: "PK-0039", customer: "Isabel Torres", email: "isabel.torres@hotmail.com", lockerId: "LCK-112I", phone: "+34 622 334 455", origin: "Sevilla, Spain", destination: "Santa Clara, Cuba", date: "2025-02-04", status: "Pending" },
  { id: "PK-0038", customer: "Miguel Ruiz", email: "miguel.ruiz@cubamail.cu", lockerId: "LCK-067J", phone: "+53 5 555 1234", origin: "Havana, Cuba", destination: "Panama City, Panama", date: "2025-01-28", status: "Cancelled" },
];
...
const AdminShipments: React.FC = () => {
  ...
  const handleAssign = (id: string) => {
    ...
    setNotice(`Pickup ${id} assigned successfully.`);
  ...
  const filteredPickups = useMemo(() => {
    ...
    return pickups.filter(p => {
      ...
      if (term === "") return true;
      const hay = `${p.id} ${p.customer} ${p.email} ${p.lockerId} ${p.phone} ${p.origin} ${p.destination}`.toLowerCase();
  ...
  return (
    <div className="p-4 md:p-8 bg-gradient-to-br from-gray-50 via-white to-gray-50 min-h-screen">
      <Helmet><title>Admin Shipments | EXPRESUR</title></Helmet>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={resetFilters} className="text-sm px-4 py-2 border rounded-lg hover:bg-gray-100 transition">
            Reset Filters
          </button>
          <div className="text-sm px-4 py-2 rounded-lg bg-gray-100">
            Showing {filteredPickups.length} of {pickups.length}
          </div>
        </div>
      </div>
      ...
      {/* Notice */}
      {notice && (
        <div className="mb-6 p-4 rounded-xl bg-green-50 border border-green-200 text-green-800 flex items-center gap-2">
          <span>{notice}</span>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search ID, name, email, locker, phone..."
          ...
          <button className="flex items-center justify-center gap-2 bg-green-700 text-white px-6 py-3 rounded-xl hover:bg-green-800 transition shadow-md">
            <Filter size={18} />
            Apply
          </button>
        </div>
      ...
      <div className="bg-white/90 backdrop-blur-sm border border-gray-200 rounded-2xl shadow-xl overflow-hidden">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-bold text-gray-800">All Shipments</h2>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50/80 text-gray-700 uppercase text-xs tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 text-left">Email</th>          {/* NEW */}
                <th className="px-6 py-4 text-left">Locker ID</th>     {/* NEW */}
                ...
                <tr key={pickup.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-5 text-gray-700">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{pickup.email}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5 font-medium text-purple-700">
                    <div className="flex items-center gap-2">
                  ...
                  <td className="px-6 py-5 text-gray-700">
                    <div>{pickup.origin} → <span className="font-medium text-green-700">{pickup.destination}</span></div>
                  </td>
                  <td className="px-6 py-5 hidden lg:table-cell text-gray-600">
                    <div className="flex items-center gap-2">
                      <Calendar size={14} />
                  ...
                  <td className="px-6 py-5 text-center">
                    <div className="flex items-center justify-center gap-4">
                      <button
                        ...
                        className="text-blue-600 hover:text-blue-800 flex items-center gap-1 hover:bg-blue-50 px-3 py-1.5 rounded-lg transition"
                      >
                        <Eye size={16} /> View
                      </button>
                      ...
                      {pickup.status === "Pending" && (
                        <button
                          ...
                          className="text-green-700 hover:text-green-900 flex items-center gap-1 hover:bg-green-50 px-3 py-1.5 rounded-lg transition"
                        >
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-8">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <h3 className="text-2xl font-bold text-gray-900">{selected.id}</h3>
                  <p className="text-sm text-gray-500 mt-1">{selected.customer}</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <p className="text-sm text-gray-500 flex items-center gap-2"><Lock size={16} /> Locker ID</p>
                    <p className="font-medium text-purple-700">{selected.lockerId}</p>
                  </div>
                  <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <p className="text-sm text-gray-500 flex items-center gap-2"><Calendar size={16} /> Pickup Date</p>
                    <p className="font-medium">{formatDate(selected.date)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Status</p>
                    <span className={`inline-block px-4 py-2 rounded-full text-sm font-semibold ${getStatusColor(selected.status)}`}>
                      {selected.status}
                    </span>
                  </div>
                </div>
              ...
              <div className="mt-8 flex justify-end gap-3">
                {selected.status === "Pending" && (
                  <button
                    ...
                    className="bg-green-700 hover:bg-green-800 text-white px-5 py-3 rounded-lg flex items-center gap-2 transition"
                  >
                    <UserCheck size={18} /> Assign Pickup
                  </button>
                )}
                <button onClick={closeModal} className="px-5 py-3 border rounded-lg hover:bg-gray-50 transition">
                  Close
                </button>
              </div>
            </div>
