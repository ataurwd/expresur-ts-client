...
const AdminLogistic = () => {
  ...
  // Modal State
  const [modal, setModal] = useState<{ open: boolean; mode: 'view' | 'create'; data: LogisticsGroup | null }>({
  ...
  // Dynamic Alert Stats (Groups < 70% full)
  const incompleteCount = data.filter(item => (item.currentWeight / item.maxWeight) < 0.7 && item.status === 'Active').length;
  ...
  const handleAddPackage = (id: string) => {
    // Simulates adding a package to a specific group
    ...
    if (weightToAdd && !isNaN(weightToAdd)) {
      setData(prev => prev.map(item => {
        if (item.id === id) {
          ...
          return { ...item, currentWeight: newWeight, packageCount: item.packageCount + 1 };
        }
        return item;
      }));
    }
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] p-6 md:p-10 font-sans text-gray-800">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-[28px] font-bold text-[#111827] tracking-tight leading-tight">Logistics Groups</h1>
      ...
      {/* --- ALERT BOX --- */}
      {incompleteCount > 0 ? (
        <div className="bg-[#FEFCE8] border border-[#FEF9C3] rounded-2xl p-6 mb-10 transition-all">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <div className="flex items-center gap-2">
          {['All', 'Maritime', 'Air', 'Express'].map(tab => (
            <button 
              key={tab}
        ...
        <button onClick={() => handleOpenModal(null, 'create')} className="bg-[#166534] hover:bg-[#14532d] text-white px-5 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition shadow-sm w-full sm:w-auto justify-center">
...
/* --- CARD COMPONENT --- */
const GroupCard = ({ data, onView, onAdd }: { data: LogisticsGroup, onView: () => void, onAdd: () => void }) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-50 flex flex-col h-full hover:shadow-md transition-shadow">
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
             <h3 className="text-gray-600 font-medium text-lg truncate w-40">{data.name}</h3>
      ...
      <div className="mt-auto">
        ...
        <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden mb-2">
          <div 
            className={`h-full rounded-full transition-all duration-500 ${percent < 80 ? 'bg-[#22c55e]' : percent < 95 ? 'bg-orange-400' : 'bg-[#ef4444]'}`} 
            style={{ width: `${Math.min(percent, 100)}%` }}
          ></div>
        </div>

        <div className="text-right text-xs text-gray-400 mb-6">
          {percent}% full • {data.packageCount} packages
        </div>

        {/* Buttons */}
        <div className="flex gap-3">
          <button onClick={onView} className="flex-1 bg-[#166534] hover:bg-[#14532d] text-white py-2.5 rounded-lg text-sm font-semibold transition-colors">
            View Details
          </button>
          <button onClick={onAdd} className="px-4 border border-gray-100 rounded-lg flex items-center gap-2 text-gray-400 hover:bg-gray-50 transition-colors text-sm font-medium">
            <Package size={16} /> Add
          </button>
        </div>
      </div>
...
/* --- MODAL COMPONENT --- */
const GroupModal = ({ mode, data, onClose, onSave, onDelete }: any) => {
  const [formData, setFormData] = useState<LogisticsGroup>(data);
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave(formData); }} className="p-6 space-y-4">
           ...
           <div>
              ...
              <input name="name" value={formData.name} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none" />
           </div>

           <div className="grid grid-cols-2 gap-4">
             <div>
               <label className="block text-xs font-medium text-gray-500 uppercase mb-1">Type</label>
               <select name="type" value={formData.type} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-green-500 outline-none">
                 <option>Express</option><option>Air</option><option>Maritime</option>
               </select>
             </div>
             <div>
               <label className="block text-xs font-medium text-gray-500 uppercase mb-1">Status</label>
               <select name="status" value={formData.status} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-green-500 outline-none">
                 <option>Active</option><option>Departed</option>
               </select>
             </div>
           </div>

           <div className="grid grid-cols-2 gap-4">
             <div><label className="block text-xs font-medium text-gray-500 uppercase mb-1">Origin</label><input name="origin" value={formData.origin} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none" /></div>
             <div><label className="block text-xs font-medium text-gray-500 uppercase mb-1">Destination</label><input name="destination" value={formData.destination} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none" /></div>
           </div>

           <div className="grid grid-cols-2 gap-4">
             <div><label className="block text-xs font-medium text-gray-500 uppercase mb-1">Max Weight (kg)</label><input type="number" name="maxWeight" value={formData.maxWeight} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none" /></div>
             <div><label className="block text-xs font-medium text-gray-500 uppercase mb-1">Current Weight</label><input type="number" name="currentWeight" value={formData.currentWeight} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none" /></div>
           </div>
           ...
           <div className="flex justify-between pt-4 mt-2 border-t border-gray-100">
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" className="bg-[#166534] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#14532d] flex items-center gap-2"><Save size={16} /> Save</button>
              </div>
           </div>
        </form>
      </div>
